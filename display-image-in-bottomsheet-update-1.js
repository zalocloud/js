...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const imagesList = []; // Danh sách các URL ảnh được hỗ trợ
  ...
  document.addEventListener('click', function (event) {
    ...
    // Kiểm tra nếu là hình ảnh và nó thuộc domain hỗ trợ
    if (imageElement.tagName === 'IMG' && isSupportedImageUrl(imageElement.src)) {
      openPopupSheet(imageElement.src); // Mở bottom sheet và chèn hình ảnh vào
    }
  });

  // Kiểm tra URL hình ảnh có phải domain hỗ trợ không
  function isSupportedImageUrl(url) {
    return url.includes('imgur.com') || url.includes('res.cloudinary.com') || url.includes('cloudinary.com');
  }
  ...
  function openPopupSheet(imageUrl) {
    ...
    // Lấy tất cả các ảnh hỗ trợ từ trang và thêm vào danh sách
    document.querySelectorAll('img').forEach((img) => {
      if (isSupportedImageUrl(img.src) && !imagesList.includes(img.src)) {
  ...
  // ✅ Đóng bottom sheet khi nhấn phím Esc
    // ✅ Đóng bottom sheet khi nhấn phím Esc (mô phỏng click vào nút đóng)
  document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && isPopupOpen) {
      e.preventDefault();
      // Giả lập hành vi y hệt nút đóng — gọi mọi handler của nút
